...
function EditTopupProduct(props) {
  ...
  // Optional label shown in front of the storefront quantity input for
  // any package under this product that has `allow_quantity = 1`. Capped
  // at 64 chars to match the DB column. Loaded from `data` once the
  // product fetch resolves (see useEffect below). Blank ⇒ storefront
  // falls back to the default "Quantity" label.
              ...
              {hasData(data, loading, error) && (
                <div>
                  ...
                  {/* Quantity label. Quantity itself is enabled per-package
                      ...
                      field is just the prefix used for the storefront
                      input legend so all quantity-enabled packages under
                      the same product share one label. */}
                  <div className="my-2">
                    <label
