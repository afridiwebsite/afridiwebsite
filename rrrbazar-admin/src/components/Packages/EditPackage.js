...
function EditPackage(props) {
  ...
  // Quantity / pricing sale style — see AddPackage for the value meanings.
  ...
  useEffect(() => {
    if (!data) return;
    // Resolve the sale style from the saved flags: off ⇒ none, else the
    // explicit quantity_mode ('range' / 'amount').
    if (Number(data.allow_quantity) !== 1) {
      setQuantitySystem("none");
    } else {
      setQuantitySystem(
        String(data.quantity_mode || "amount").toLowerCase() === "range"
          ? "range"
          : "amount",
      );
    }
    ...
    // Parse stored ranges (JSON string or array) back into editable rows.
    let parsedRanges = [];
    try {
      const raw = data.dollar_ranges;
      const arr =
        typeof raw === "string"
          ? JSON.parse(raw || "[]")
          : Array.isArray(raw)
            ? raw
            : [];
      parsedRanges = (Array.isArray(arr) ? arr : []).map((r) => ({
        _key: Math.random().toString(36).slice(2),
        lower_taka: r?.lower_taka != null ? String(r.lower_taka) : "",
        upper_taka: r?.upper_taka != null ? String(r.upper_taka) : "",
        lower_dollar: r?.lower_dollar != null ? String(r.lower_dollar) : "",
        upper_dollar: r?.upper_dollar != null ? String(r.upper_dollar) : "",
        price: r?.price != null ? String(r.price) : "",
      }));
    } catch {
      parsedRanges = [];
    }
    setDollarRanges(parsedRanges);
  ...
  // See AddPackage — every range row needs a usable band + positive price.
